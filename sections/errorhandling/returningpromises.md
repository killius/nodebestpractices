...
Regular stacktraces was always cutting on the most recent `await` of the execution flow since async/await was introduced
in Node.js 7.6. In Node.js 10 a feature called "zero-cost async stacktraces" has been added (unflagged in Node.js 12)
which goal is to add async functions calls to the stacktraces. But due to non-trivial implementation details, functions
that returns promises without explicit `await` would still be missed in stacktraces when the returned promise would be
rejected. So in order to avoid holes in stacktraces promises should always be explicitly resolved with `await` before
been returned from functions
